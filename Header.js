import React, { useState, useEffect, useContext } from 'react'
import './Header.css'
import { CircularProgressbar } from 'react-circular-progressbar';
import { themeContext } from './App.js'

function Header() {

    const darkMode = useContext(themeContext)
    const [option, setOption] = useState('BTC')
    const [click1, setClick1] = useState(false)
    const [click2, setClick2] = useState(false)
    const [click3, setClick3] = useState(false)
    const [timer, setTimer] = useState(60)

    useEffect(() => {
        setTimeout(() => {
            if (timer == 0) {
                setTimer(60)
            }
            else {
                setTimer(timer - 1)
            }
            return <div>{timer}</div>
        }, 1000);
    }, [timer])

    return (
        <div className='header'>
            <div className="headerLeft">
                <img src="https://hodlinfo.com/static/media/HODLINFO.8f78fc06.png" alt="Hodlinfo" id='hodlinfo' />
                <p className="poweredBy">Powered by <a href='https://www.finstreet.in/' target="_blank">Finstreet</a></p>
            </div>
            <div className="headerCenter">
                <select className={darkMode ? (`btn-dark ${click1 && 'click-dark'}`) : (`btn ${click1 && `click`}`)} name="currency" id="currency" onClick={() => setClick1(!click1)}>
                    <option value="volvo">INR</option>
                </select>
                <select className={darkMode ? (`btn-dark ${click2 && 'click-dark'}`) : (`btn ${click2 && `click`}`)} name="crypto" id="crypto" onChange={e => setOption(e.target.value)} onClick={() => setClick2(!click2)}>
                    <option value="BTC">BTC</option>
                    <option value="ETH">ETH</option>
                    <option value="UHDT">UHDT</option>
                    <option value="XRP">XRP</option>
                    <option value="TRX">TRX</option>
                    <option value="DASH">DASH</option>
                    <option value="ZEC">ZEC</option>
                    <option value="XEM">XEM</option>
                    <option value="IOST">IOST</option>
                    <option value="WIN">WIN</option>
                    <option value="BTT">BTT</option>
                    <option value="WRX">WRX</option>
                </select>
                {/* <div className="dropdown">
                    <button onClick={()=>setDropDown(!dropDown)} className={`btn ${dropDown&&`dropbtn`}`}>Dropdown</button>
                    <div id="myDropdown" className="dropdown-content">
                        <p>INR</p>
                    </div>
                </div> */}
                <a href="https://wazirx.com/invite/sp7pvbt6?utm_source=finstreet&utm_medium=affiliate&utm_campaign=regnow-btn" target='_blank'>
                    <button className={darkMode ? (`btn-dark ${click3 && 'click-dark'}`) : (`btn ${click3 && `click`}`)} onClick={() => setClick3(!click3)}> BUY {option}</button >
                </a>
            </div>
            <div className="headerRight">
                <CircularProgressbar value={timer} maxValue={60} minValue={0} text={timer} />
                <a href="_" >
                    <div className="telegram">
                        <img src="data:image/png;base64," alt="telegram" />
                        <p>Connect Telegram</p>
                    </div>
                </a>
            </div>
        </div >
    )
}

export default Header
